package com.test.helpers.utils.curlLoggingUtils;
import com.test.helpers.report.*;
    ...
    public void process(HttpRequest request, HttpContext context) throws HttpException, IOException {
        try {
            ...
            if(CurlLoggingInterceptor.captureAPIDetails) {
                ReportHelper.getInstance().setCurlFilePath(FileHelper.getInstance().createCurlFile(message.toString()));
                log.debug(message.toString());
            }
        } catch (Exception e) {
            log.warn("Failed to generate CURL command for HTTP request", e);
            ReportHelper.getInstance().setCurlFilePath(FileHelper.getInstance().createCurlFile("Failed to generate CURL command for HTTP request"));
        }
    }
